...
# Cargo valores
time_node_0 = data['vectime'].loc[[24]].tolist()
buffer_node_0 = data['vecvalue'].loc[[24]].tolist()

time_node_2 = data['vectime'].loc[[25]].tolist()
buffer_node_2 = data['vecvalue'].loc[[25]].tolist()

time_node_7 = data['vectime'].loc[[26]].tolist()
buffer_node_7 = data['vecvalue'].loc[[26]].tolist()

time_node_1 = data['vectime'].loc[[27]].tolist()
buffer_node_1 = data['vecvalue'].loc[[27]].tolist()

time_node_6 = data['vectime'].loc[[28]].tolist()
buffer_node_6 = data['vecvalue'].loc[[28]].tolist()

time_saltos = data['vectime'].loc[[29]].tolist()
buffer_saltos = data['vecvalue'].loc[[29]].tolist()

time_node_5_app = data['vectime'].loc[[30]].tolist()
buffer_node_5_app = data['vecvalue'].loc[[30]].tolist()
...
max_node0 = max(buffer_node_0)
xpos = buffer_node_0.index(max_node0)
xmax = time_node_0[xpos]

ax1 = plt.subplot(212)
plt.suptitle("Caso 1: InterArrivalTime = 1", fontsize=20)
ax1.plot(time_node_0, buffer_node_0, linewidth=1.0, linestyle="-", color='gray')
ax1.plot(xmax, max_node0, 'o', color='red', label=f"Max:{round(xmax, 2), round(max_node0, 2)}")
...
ax1 = plt.subplot(212)
plt.suptitle("Caso 1: InterArrivalTime = 1", fontsize=20)
ax1.plot(time_node_5_app, buffer_node_5_app, linewidth=1.0, linestyle="-", color='gray')
...
ax2 = plt.subplot(221)
ax2.plot(time_node_6, buffer_node_6, linewidth=1.0, linestyle="-", color='blue')
ax2.set_title('Node[6]')
plt.grid()

ax3 = plt.subplot(222)
ax3.plot(time_node_7, buffer_node_7, linewidth=1.0, linestyle="-", color='green')
...
plt.title('Saltos node[5]')
plt.suptitle("Caso 1: InterArrivalTime = 1", fontsize=20)
plt.plot(time_saltos, buffer_saltos, linewidth=1.0, linestyle="-", 
         color='blue', label=f"Saltos ({total})")
plt.hlines(saltos_averg, 0, 200, linestyles='--', color='red',
           label=f"Promedio: {round(float(saltos_averg), 2)}s")
plt.xlabel('Tiempo de simulacion en segundos')
plt.ylabel('Saltos')
plt.legend()
plt.grid()
plt.show()
